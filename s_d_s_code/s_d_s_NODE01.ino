...
const int MPU = 0x68;
int16_t AcX, AcY, AcZ1, Tmp, GyX, GyY, GyZ;
void setup() {
  Wire.begin();
...
void loop() {
 ...
 unsigned long acc1value = AcZ1;
  RF24NetworkHeader header(node00);    // (Address where the data is going)
  bool ok = network.write(header, &acc1value, sizeof(acc1value)); // Send the data
