/*
  Sogno_di_sogni_NODE0 - == Base/ Master Node 02==
...
const int MPU = 0x68;
int16_t AcX, AcY, AcZ2, Tmp, GyX, GyY, GyZ;
...
void loop() {
  ...
  unsigned long acc2value = AcZ2;
  RF24NetworkHeader header(node00);    // (Address where the data is going)
  bool ok = network.write(header, &acc2value, sizeof(acc2value)); // Send the data
